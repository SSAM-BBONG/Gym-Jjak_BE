...
>
> **구현 예정 계약:** 아래 destination과 이벤트는 신규 `chatbot` 도메인 구현 시 추가합니다. 현재 기존 `chat` WebSocket에는 존재하지 않습니다.
...
모든 이벤트는 `type`, `sessionId`, `requestId`를 포함합니다. 새 세션은 첫 번째 `started` 이벤트에서 클라이언트가 보관할 `sessionId`를 받습니다.
...
| `CHATBOT_SUBSCRIPTION_REQUIRED` | FastAPI가 구독 자격을 거부함 |
