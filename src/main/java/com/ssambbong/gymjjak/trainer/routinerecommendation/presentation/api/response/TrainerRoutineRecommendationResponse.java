...
public record TrainerRoutineRecommendationResponse(
    ...
    public record Day(String dayLabel, String goal, List<String> warmUp, List<Exercise> exercises, List<String> coolDown) {
        static Day from(TrainerRoutineRecommendationResult.Day day) {
            return new Day(day.dayLabel(), day.goal(), day.warmUp(), day.exercises().stream().map(Exercise::from).toList(), day.coolDown());
        }
    }
    public record Exercise(String name, String part, int sets, String reps, String intensity, int restSeconds, String rationale) {
        static Exercise from(TrainerRoutineRecommendationResult.Exercise exercise) {
            return new Exercise(exercise.name(), exercise.part(), exercise.sets(), exercise.reps(), exercise.intensity(), exercise.restSeconds(), exercise.rationale());
        }
    }
