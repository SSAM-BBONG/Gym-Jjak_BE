...
> OCR의 `fields`가 빈 배열인 것은 OCR 도메인 자체에서는 정상 응답으로 허용합니다. 다만 트레이너 신청의 필수 자격증 정책에서는 요구 필드가 없으므로 `TRAINER_APPLICATION_400_2`로 실패합니다.
...
Clova 응답에서 첫 번째 `images[0]`만 사용하며, `inferResult`가 `SUCCESS`여야 합니다. 응답에 `images`가 없거나 비어 있거나, `fields`가 `null`이면 유효하지 않은 OCR 응답으로 처리합니다.
...
| `502 Bad Gateway` | `OCR_502_2` | OCR 응답 형식이 올바르지 않습니다. | `images` 누락·빈 배열, `inferResult != SUCCESS`, `fields == null`인 경우 |
| `400 Bad Request` | `TRAINER_APPLICATION_400_2` | 필수 자격증 OCR 검증에 실패했습니다. | OCR 호출은 성공했으나 트레이너 신청의 템플릿·필수 필드 정책을 통과하지 못한 경우 |
