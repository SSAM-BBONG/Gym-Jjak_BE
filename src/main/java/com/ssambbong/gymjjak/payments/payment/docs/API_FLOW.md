...
sequenceDiagram
    ...
    participant V as PortOneWebhookVerifier
    participant S as PaymentCommandService
    participant PO as PortOnePaymentVerifyPort
    participant DB as PaymentRepository

    P->>WC: POST /api/payments/webhook (webhook-id, webhook-timestamp, webhook-signature 헤더 + body)
    WC->>V: verify(rawBody, webhook-id, webhook-timestamp, webhook-signature)
    alt 서명 검증 실패
        V-->>WC: WebhookInvalidSignatureException
        WC-->>P: 400
    end
    WC->>S: ProcessWebhookCommand (rawBody 파싱 후 전달)
